...
Locally generated connections to common IMAP/POP3/SMTP ports (`143`, `993`, `110`, `995`, `25`, `465`, `587`) are marked `0x6d100000` by chain `WGOM_MAIL`, jumped from `OUTPUT` in the mangle table. That is the same mark WireGuard's own UDP socket carries, so mail traffic is treated identically: it never matches the `not fwmark tunnelMark` rule and stays on the unchanged main table instead of `wgom0`. This keeps mail client and server connections outside the tunnel even while other traffic is routed through it. It is controlled by `Settings.MailBypass` (`-mail-bypass`, default `true`); `ensureFirewallFamily` installs `WGOM_MAIL` when enabled and removes it via `removeMailBypass` when disabled, so toggling the flag and restarting reconciles the chain on the next heartbeat.
